...
describe('Users Database Operations (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        await app.init();
        usersService = moduleFixture.get(users_service_1.UsersService);
    ...
    describe('User Service Integration', () => {
        it('should find user by email via service', async () => {
            const created = await test_helpers_1.DatabaseHelpers.createTestUser(userModel, {
                email: 'findbyemail@example.com',
                entityId: testEntity._id,
                tenantId: testEntity._id,
            });
            const found = await userModel.findOne({ email: 'findbyemail@example.com' });
            expect(found).toBeDefined();
