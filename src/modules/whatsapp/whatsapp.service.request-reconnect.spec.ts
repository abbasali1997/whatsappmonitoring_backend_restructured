...
describe("WhatsAppService.requestReconnect", () => {
  it("should queue reconnect when service bus is configured", async () => {
    ...
    const mockConfigService = { get: jest.fn((key: string, def?: any) => {
      if (key === "azure.serviceBus.connectionString") return "Endpoint=sb://fake";
    ...
    expect(mockQueueService.queueReconnectSession).toHaveBeenCalledWith("session-123");
  });
  ...
  it("should call initializeClient when service bus is not configured", async () => {
    ...
    const spy = jest.spyOn(svc as any, "initializeClient").mockResolvedValue(undefined);
